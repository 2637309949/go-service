...
func (g *web) GenerateImports(file *generator.FileDescriptor, imports map[generator.GoImportPath]generator.GoPackageName) {
	if len(file.FileDescriptorProto.Service) == 0 {
		return
	}
	g.P("import (")
...
func (g *web) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
	...
	*/
	...
	// Server registration.
	g.P("func Register", servName, "Handler(s ", webPkg, ".Service, hdlr ", serverType, ", opts ...", webPkg, ".Option) {")
	for _, method := range service.Method {
		if method.Options != nil && proto.HasExtension(method.Options, options.E_Http) {
			g.P("opts = append(opts, ", webPkg, ".Endpoint(&", apiPkg, ".Endpoint{")
			g.generateEndpoint(servName, method)
			g.P("}))")
		}
	}
...
func (g *web) generateEndpoint(servName string, method *pb.MethodDescriptorProto) {
	if method.Options == nil || !proto.HasExtension(method.Options, options.E_Http) {
		return
	}
	// http rules
...
func (g *web) generateClientSignature(servName string, method *pb.MethodDescriptorProto) string {
	origMethName := method.GetName()
	methName := generator.CamelCase(origMethName)
	if reservedClientName[methName] {
		methName += "_"
	}
	reqArg := ", w " + g.typeName(method.GetInputType())
	if method.GetClientStreaming() {
		reqArg = ""
	}
	respName := "r *" + g.typeName(method.GetOutputType())
	if method.GetServerStreaming() || method.GetClientStreaming() {
		respName = servName + "_" + generator.CamelCase(origMethName) + "Service"
	}

	return fmt.Sprintf("%s(ctx %s.Context%s, %s)", methName, contextPkg, reqArg, respName)
}

func (g *web) generateClientMethod(pkg, reqServ, servName, serviceDescVar string, method *pb.MethodDescriptorProto, descExpr string) {
	...
	if !method.GetServerStreaming() && !method.GetClientStreaming() {
		g.P("next, err := c.c.Options().Selector.Select(c.name)")
		g.P("if err != nil { return }")
		g.P("s, err := next()")
		g.P("if err != nil { return }")
		g.P(`service := fmt.Sprintf("http://%s", s.Address)`)
		g.P("if len(service) == 0 {")
		g.P("w.WriteHeader(404)")
		g.P("return")
		g.P("}")
		g.P("rp, err := url.Parse(service)")
		g.P("if err != nil {")
		g.P("w.WriteHeader(500)")
		g.P("return")
		g.P("}")
		g.P("httputil.NewSingleHostReverseProxy(rp).ServeHTTP(w, r)")
		g.P("}")
		g.P()
		return
	}
...
func (g *web) generateServerSignature(servName string, method *pb.MethodDescriptorProto) string {
	origMethName := method.GetName()
	methName := generator.CamelCase(origMethName)
	if reservedClientName[methName] {
		methName += "_"
...
func (g *web) generateServerMethod(servName string, method *pb.MethodDescriptorProto) string {
	methName := generator.CamelCase(method.GetName())
	...
	if !method.GetServerStreaming() && !method.GetClientStreaming() {
		g.P("func (h *", unexport(servName), "Handler) ", methName, "(ctx ", contextPkg, ".Context, w ", inType, ", r *", outType, ") {")
		g.P("h.", serveType, ".", methName, "(ctx, w, r)")
		g.P("}")
		g.P()
		return hname
	}
